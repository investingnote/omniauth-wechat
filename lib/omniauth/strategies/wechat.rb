...
module OmniAuth
  module Strategies
    class Wechat < OmniAuth::Strategies::OAuth2
      ...
      def raw_info
        @uid ||= access_token['unionid']
        oid = access_token['openid']
        @raw_info ||= begin
          access_token.options[:mode] = :query
          if access_token['scope'] == 'snsapi_login'
            response = access_token.get('/sns/userinfo',
                                        params: { 'openid': oid, 'lang': 'zh_CN' }, parse: :text)
            @raw_info = JSON.parse(response.body.gsub(/[\u0000-\u001f]+/, ''))
          else
            @raw_info = { 'openid': oid, 'unionid': @uid }
            @raw_info
          end
        end
      end
